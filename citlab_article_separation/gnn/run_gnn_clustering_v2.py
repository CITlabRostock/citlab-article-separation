...
from citlab_article_separation.gnn.input.feature_generation import is_aligned_horizontally_separated, \
    is_aligned_heading_separated, discard_text_regions_and_lines
from citlab_article_separation.gnn.clustering.textblock_clustering import TextblockClustering
from citlab_article_separation.gnn.io import save_clustering_to_page, save_conf_to_json
...
def mask_horizontally_separated_confs(confs, page_path):
    ...
    if FLAGS.mask_horizontally_separated_confs and 'SeparatorRegion' not in regions and \
            not FLAGS.mask_heading_separated_confs:
        ...
        for j in range(i + 1, num_text_regions):
            ...
            if FLAGS.mask_heading_separated_confs:
                if is_aligned_heading_separated(tr_i, tr_j):
                    logger.debug(f"Pair ({i}, {j}) separated by heading. "
            ...
            if FLAGS.mask_horizontally_separated_confs and separator_regions:
                if is_aligned_horizontally_separated(tr_i, tr_j, separator_regions):
                    logger.debug(f"Pair ({i}, {j}) horizontally separated. "
                                 f"Previous confs: ({i}, {j}) = {confs[i, j]:.4f}, ({j}, {i}) = {confs[j, i]:.4f}")
                    masked[i, j] = 0
                    masked[j, i] = 0
...
def gnn_clustering(json_paths):
    ...
    logger.info(f"pb_path is {pb_path}")
    ...
    with sess.graph.as_default() as graph:
        ...
        dataset = input_fn.get_dataset_from_file_paths(json_paths)
        ...
        while True:  # Loop until dataset is empty
            ...
            # (IndexError for empty page_paths list, OutOfRangeError for empty tf dataset)
                ...
                break
        logger.info(f"Time: {time.time() - start_timer:.2f} seconds")
