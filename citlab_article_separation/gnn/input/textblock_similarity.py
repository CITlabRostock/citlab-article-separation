...
class TextblockSimilarity(object):
    ...
    def _calc_block_scores(self):
        ...
        for (tbKey, tbData) in self._tb_dict.items():
            ...
            if tb_size < self.min_tb_len:
                logger.debug(f"ignoring textblock {tbKey} with only {tb_size} words")
            else:
                logger.debug(f"processing textblock {tbKey} with {tb_size} words")
        ...
        logger.debug(f'evaluated {len(self._scoreDict)} textblocks …')
    ...
    def _calc_edge_scores(self):
        ...
        logger.debug(f'compared {count_pairs} textblock pairs …')
