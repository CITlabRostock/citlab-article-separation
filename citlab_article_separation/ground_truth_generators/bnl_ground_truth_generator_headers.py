...
class BNLGroundTruthGenerator(TextBlockGroundTruthGenerator):
    def __init__(self, path_to_img_lst, max_resolution=(0, 0), scaling_factor=1.0, use_bounding_box=False,
        ...
        self.heading_title_regions = self.get_classic_heading_regions_list(["", "title", "subheadline", "overline"])
    ...
    def create_ground_truth_images(self):
        ...
        for i in range(len(self.img_path_lst)):
            ...
            regions = [self.title_headline_regions[i], self.title_subheadline_regions[i], self.title_other_regions[i], self.heading_title_regions[i]]
            if all(len(region) == 0 for region in regions):
                continue
